...
class NoteDiscussionForm : AbstractQuestForm(), TakePhotoFragment.Listener {
    ...
    private var closeNote: MutableState<Boolean> = mutableStateOf(false)
    ...
    override fun onClickOk() {
        val close = closeNote.value
        require(noteText.value.isNotBlank() || close) { "NoteQuest has been answered with an empty comment!" }
        viewLifecycleScope.launch {
            withContext(Dispatchers.IO) {
                val action = if (close) NoteEditAction.CLOSE else NoteEditAction.COMMENT
                noteEditsController.add(noteId, action, geometry.center, noteText.value, noteImagePaths.value)
            }
